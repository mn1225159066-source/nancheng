...
class FanqieScraper:
    ...
    def get_chapter_content_cdp(self, chapter_url):
        ...
        try:
            ...
            create_res = json.loads(ws.recv())
            target_id = create_res.get('result', {}).get('targetId')
            if not target_id:
                ws.close()
                return None
            ...
            if not session_id:
                ws.close()
                return None
            def send_session(cmd_id, method, params=None):
                msg = {"id": cmd_id, "method": method, "sessionId": session_id}
                if params:
                    msg["params"] = params
                ws.send(json.dumps(msg))
                return json.loads(ws.recv())
            send_session(3, "Page.enable")
            send_session(4, "Network.enable")
            send_session(5, "Page.navigate", {"url": chapter_url})
            import time as _t
            start = _t.time()
            while _t.time() - start < 10:
                try:
                    ...
                    break
            res = send_session(6, "Runtime.evaluate", {"expression": "document.querySelector('.muye-reader-content')?.innerText || ''"})
            text_val = res.get('result', {}).get('result', {}).get('value')
            res2 = send_session(7, "Runtime.evaluate", {"expression": "(document.documentElement.outerHTML.match(/https?:[^'\"]+\\.woff2/)||[])[0]||''"})
            font_val = res2.get('result', {}).get('result', {}).get('value')
            try:
                ws.send(json.dumps({"id": 8, "method": "Target.closeTarget", "params": {"targetId": target_id}}))
                ws.recv()
            except Exception:
                pass
            ws.close()
            if text_val:
                html = '<pre>' + text_val + '</pre>'
                return {"content_html": html, "font_url": font_val}
            return None
        except Exception:
            return None
